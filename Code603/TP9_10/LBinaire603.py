...
class LBinaire603(list):#Voir Object ?
    ...
    def lFrequences(self):
        ...
        >>> lf=LBinaire603([1,2,3,3,3,3,4,8,8,128]).lFrequences()
        >>> lf[3]
        0.4
        """
        lf=[0]*256
    ...
    def entropie(self):
        """
        >>> LBinaire603([6,6,6,6,6,6,6,6,5,5,5,5,5,5,7,7,7,7,8,9]).entropie()
        1.9464393446710155
        """
        h=0
        lf = self.lFrequences()
    ...
    def afficheHistogrammeDesFrequences(self,titre="Histogramme des fréquences",nbValMax=20):
        """
        LBinaire603([1,2,3,3,3,3,4,8,8,128]).afficheHistogrammeDesFrequences()
    ...
    def demo():
        lb=LBinaire603([1,2,3,3,3,3,4,8,8,128])
        lf=lb.lFrequences()
        print(lf[3])
    ...
    def demoEntropie():
        for ext in ["txt","odt","pdf","py","odp","zip","jpg","png","pcx","bmp"]:
            ...
            print(f"L'entropie du fichier {'Exemple.'+ext} est de : {LBinaire603.bin603DepuisFichier('Exemple.'+ext).entropie():1.3f}")

        for k in range(5):
            monBin=LBinaire603.exBin603(num=k,taille=2000) #Bug de clé trop longue pour taille =100000
            print("Bin:",repr(monBin))
            monBin.affFreq()
    def demoLivres():
        for nf in ["Les Miserables","Notre Dame de Paris","L Education sentimentale","Guerre et Paix"]:
            lb=LBinaire603.bin603DepuisFichier(nf+".TXT")
...
if __name__ == "__main__":
    ...
    print(f"Liste des fréquences de {lb=} :")
    for k,f in enumerate(lb.lFrequences()):
        if f>0:
            print(f"0x{k:02x}:{f:0.2f}  " ,end='' )
    print()

    #LBinaire603.demoLivres()
